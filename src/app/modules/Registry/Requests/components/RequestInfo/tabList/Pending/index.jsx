...
export default (props) => {
    ...
    const semesterHeader = (department, heading, index) => (
        <Row gutter={20}>
            <Col span={24} className="c-default">{department}</Col>
            <Col flex="auto">{heading}</Col>
            <Col flex="180px"><Button type="link" className="c-white">View Details</Button></Col>
        </Row>
    )
    ...
    return (
        <Row gutter={[20, 30]} align='bottom'>
            <Col span={24}>
                <Collapse activeKey={panelActive} accordion={true} onChange={callback} className='red-card-collapse' expandIconPosition='right' bordered={false}>
                    ...
                    <Fragment key={item.id}>
                        <Panel forceRender={true} header={semesterHeader(item.department, item.structure_name, index)}>
                            ...
                            {!rejection && (
                                <Row gutter={20}>
                                    <Col span={12}>
                                        <Button size='large' type='primary' htmlType='submit' className='w-100 green-btn'>Approve</Button>
                                    </Col>
                                    <Col span={12}>
                                        <Button size='large' type='primary' htmlType='submit' onClick={() => setRejection(true)} className='w-100 red-btn'>Reject</Button>
                                    </Col>
                                </Row>
                            )}
                            {rejection && (
                                <Row gutter={20}>
                                    ...
                                    <Col span={12}>
                                        <Button size='large' type='primary' htmlType='submit' onClick={() => setRejection(false)} className='w-100 black-btn'>Cancel</Button>
                                    </Col>
                                    <Col span={12}>
                                        <Button size='large' type='primary' htmlType='submit' className='w-100 red-btn'>Reject</Button>
                                    </Col>
                                </Row>
                            )}
                        </Panel>
